...
export async function telegramWebhookHandler(req: Request, res: Response): Promise<void> {
  try {
    ...
    if (message.text && message.text.startsWith("/start")) {
      await sendTelegramMessage(chatId, "Welcome to BayonHub! 🇰🇭\n\nPlease tap the button below to share your contact so we can verify your phone number and send your OTP.", {
        reply_markup: {
          keyboard: [
            [{ text: "📱 Share Contact to Get OTP", request_contact: true }]
          ],
    ...
    if (message.contact && message.contact.phone_number) {
      ...
      if (storedOtp) {
        await sendTelegramMessage(chatId, `Your BayonHub verification code is:\n\n*${storedOtp}*\n\nEnter this code on the website to log in.`, {
          parse_mode: "Markdown",
          reply_markup: { remove_keyboard: true }
        })
      } else {
        await sendTelegramMessage(chatId, "We couldn't find a pending login for your phone number.\n\nPlease go to bayonhub.com and request a new code first.", {
          reply_markup: { remove_keyboard: true }
        })
      }
      return
